...
import {
  ...
  offlineOwnerId,
  ownedStorageKey,
} from './propietarioSinConexion';
import { classifySyncFailure, retryDelaySeconds } from './politicaSincronizacion';
...
async function enviar(tipo, payload, token) {
  ...
  if (tipo === 'ubicacion_finca') {
    response = await fetchApi(`${API_BASE_URL}/usuarios/mi-ubicacion`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({ latitud: payload.latitud, longitud: payload.longitud, direccion: payload.direccion }),
    });
