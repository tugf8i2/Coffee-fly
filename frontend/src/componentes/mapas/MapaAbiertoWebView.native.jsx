...
let routeColor='#3214d6',routeWidth=7,route=[],completedRoute=[],markerState=[],cameraState={},routeFitKey=null,markerFitKey=null,zoomCommandId=null,routeLines=[];
...
const applyZoomCommand=()=>{const command=cameraState.zoomCommand;if(!command||command.id===zoomCommandId)return;zoomCommandId=command.id;map.setZoom(Math.max(10,Math.min(19,map.getZoom()+command.delta)),{animate:false});};
const receive=(event)=>{try{const message=JSON.parse(event.data);if(message.type==='route'){routeColor=message.routeColor||'#3214d6';routeWidth=message.routeWidth||7;route=message.route||[];completedRoute=message.completedRoute||[];cameraState={...cameraState,...message.camera};document.body.classList.toggle('dark',message.mapTheme==='dark');if(message.showNavigationControls===false)map.zoomControl.remove();else map.zoomControl.addTo(map);applyRoute();applyZoomCommand();}if(message.type==='markers'){markerState=message.markers||[];cameraState={...cameraState,...message.camera};applyMarkers();applyZoomCommand();}}catch(error){send({type:'error',fatal:false,message:error.message});}};
document.addEventListener('message',receive);window.addEventListener('message',receive);
map.getContainer().addEventListener('touchstart',()=>send({type:'camera-interaction',gesture:'touch'}),{passive:true});
...
export default function MapaAbierto({ camera = {}, completedRoute = [], fallback, mapTheme = 'day', showNavigationControls = true, markers = [], onError, onManualMove, onMapPress, onMarkerDragEnd, route = [], routeColor = '#3214d6', routeWidth = 7, style }) {
  ...
  }, [camera.bearing, camera.fitKey, camera.fitMode, camera.follow, camera.followMarkerId, camera.pitch, camera.zoom, camera.zoomCommand, markers]);
