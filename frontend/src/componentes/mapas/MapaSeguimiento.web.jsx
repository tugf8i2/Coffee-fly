...
const valid = (point) => Number.isFinite(Number(point?.latitud)) && Number.isFinite(Number(point?.longitud))
  && Number(point.latitud) >= -90 && Number(point.latitud) <= 90
  && Number(point.longitud) >= -180 && Number(point.longitud) <= 180;
...
export default function MapaSeguimiento({ deliveryId, destination, navigationRoute = [], points = [] }) {
  ...
  const trail = points.filter(valid).map((point) => ({ latitude: Number(point.latitud), longitude: Number(point.longitud) }));
